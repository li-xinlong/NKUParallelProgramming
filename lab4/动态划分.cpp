...
void read_files(vector<vector<unsigned int>>& index, vector<vector<int>>& query, LARGE_INTEGER& start, LARGE_INTEGER& end_f, LARGE_INTEGER& frequency) {
	...
	double interval = static_cast<double>(end_f.QuadPart - start.QuadPart) / frequency.QuadPart;
...
int main(int argc, char* argv[])
{
	MPI_Init(&argc, &argv);
	int QueryNum = 1000;
	const int task_chunk_size = 1;//划分粒度
	...
	if (world_rank == 0) {
		read_files(index, query, start, end_f, frequency);
	}
	...
	while (true) {
		int tasks[task_chunk_size];
		if (world_rank == 0) {
			for (int i = 0; i < task_chunk_size; ++i) {
				if (task_index < QueryNum) {
					tasks[i] = task_index++;
				}
				else {
					tasks[i] = -1; // No more tasks
				}
			}
		}
		MPI_Bcast(tasks, task_chunk_size, MPI_INT, 0, MPI_COMM_WORLD);
		...
		for (int i = 0; i < task_chunk_size; ++i) {
			if (tasks[i] != -1) {
				...
				int num = query[task].size();
				vector<unsigned int> result;
				bitset<idlength>* bits = new bitset<idlength>[num];
				for (int j = 0; j < num; j++) {
					for (int k = 0; k < index[query[task][j]].size(); k++) {
						bits[j].set(index[query[task][j]][k]);
					}
				}
				for (int j = 1; j < num; j++) {
